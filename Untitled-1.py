...
def affine_decipher(text, key_a, key_b):
    ...
    for char in text:
        code_point = ord(char)
        if 0x0021 <= code_point <= 0x007e or 0xFF21 <= code_point < 0xFF5E:
            decrypted_code_point = (mod_inverse(key_a,95)*(code_point - key_b - 0x0020))%95 + 0x0020
            result += chr(decrypted_code_point)
        else:
            result += char
    return result

plaintext="Đại học CNHNa@"
key_a=3
key_b=7
